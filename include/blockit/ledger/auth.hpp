...
namespace blockit {
        ...
        class Authenticator {
          ...
          public:
            ...
            inline bool isParticipantAuthorized(const std::string &participant_id) const {
                ...
                return authorized_participants_.find(dp::String(participant_id.c_str())) !=
            ...
            inline dp::Result<void, dp::Error> updateParticipantState(const std::string &participant_id,
                ...
                std::unique_lock lock(mutex_);
                if (authorized_participants_.find(dp::String(participant_id.c_str())) ==
                    authorized_participants_.end()) {
            ...
            inline dp::Result<std::string, dp::Error> getParticipantMetadata(const std::string &participant_id,
                ...
                if (participant_it == participant_metadata_.end()) {
                    return dp::Result<std::string, dp::Error>::err(
                        dp::Error::not_found("Participant metadata not found"));
                }
    ...
    using ledger::Authenticator;
    using ledger::AuthorizationManager;
    using ledger::DeviceManager;
    using ledger::EntityManager;
    using ledger::LedgerManager;
