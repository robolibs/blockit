...
namespace blockit {
            ...
            inline dp::Result<bool, dp::Error> isValid() const {
                ...
                if (function_.to_string().empty()) {
                    return dp::Result<bool, dp::Error>::err(
                        dp::Error::invalid_argument("Transaction function is empty"));
                }
                if (signature_.empty()) {
                    return dp::Result<bool, dp::Error>::err(
                        dp::Error::invalid_argument("Transaction signature is empty"));
                }
    ...
    using ledger::has_members;
    using ledger::has_to_string;
    using ledger::Timestamp;
