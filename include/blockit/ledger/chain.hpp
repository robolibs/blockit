    ...
    namespace ledger {
        ...
        template <typename T> class Chain {
          ...
          public:
            ...
            inline dp::Result<void, dp::Error> addBlock(const Block<T> &newBlock) {
                ...
                if (blocks_.empty()) {
                    return dp::Result<void, dp::Error>::err(
                        dp::Error::invalid_argument("Cannot add block to empty chain"));
                }
            ...
            inline dp::Result<void, dp::Error> executeCommand(const std::string &issuer_entity,
                                                              const std::string &command, const std::string &tx_id,
            ...
            inline dp::Result<std::string, dp::Error> createBlockProposal(Block<T> &block,
