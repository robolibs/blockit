...
namespace blockit {
    namespace ledger {
        ...
        struct BlockSignature {
            ...
            inline static dp::Result<BlockSignature, dp::Error> deserialize(const std::vector<uint8_t> &data) {
                ...
                sig.validator_id =
                    dp::String(std::string(data.begin() + offset, data.begin() + offset + vid_len).c_str());
                offset += vid_len;
        ...
        class PoAConsensus {
          public:
            ...
            inline dp::Result<Validator *, dp::Error> getValidatorByParticipant(const std::string &participant_id) {
                ...
                return dp::Result<Validator *, dp::Error>::err(
                    dp::Error::not_found("Validator not found for participant"));
            }
            ...
            inline dp::Result<bool, dp::Error> canPropose(const std::string &validator_id,
                ...
                if (tracker.last_proposal_time > 0 &&
                    time_since_last < (config_.min_seconds_between_proposals * 1000)) {
                    return dp::Result<bool, dp::Error>::err(
            ...
            inline dp::Result<bool, dp::Error> addSignature(const std::string &proposal_id,
          ...
          private:
            struct Proposal {
                ...
                inline bool hasQuorum() const {
                    return static_cast<int>(signed_validators.size()) >= required_signatures;
                }
            };
    ...
    using ledger::BlockSignature;
    using ledger::PoAConfig;
    using ledger::PoAConsensus;
