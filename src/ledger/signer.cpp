#include <blockit/ledger/signer.hpp>
#include <sstream>

namespace blockit::ledger {

    std::vector<unsigned char> stringToVector(const std::string &str) { return {str.begin(), str.end()}; }

    std::string vectorToString(const std::vector<unsigned char> &vec) { return {vec.begin(), vec.end()}; }
    ...
    // Crypto class implementations
    Crypto::Crypto(const std::string &keyFile) : algorithm_(lockey::Lockey::Algorithm::Ed25519), crypto_(algorithm_) {
        keypair_ = crypto_.generate_keypair();
        if (!keypair_.private_key.empty())
            hasKeypair_ = true;
    }

    std::vector<unsigned char> Crypto::sign(const std::string &data) {
        if (!hasKeypair_)
    ...
    std::string Crypto::getPublicHalf() {
        if (!hasKeypair_)
    ...
    std::vector<uint8_t> Crypto::getPublicKeyRaw() {
        if (!hasKeypair_)
            throw std::runtime_error("No keypair available");
